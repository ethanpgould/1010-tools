...
def create_period_df(pair):
	...
	out_df = pd.DataFrame(index = credit_tickers, columns = column_periods)

	for ticker in data:
		if ticker in credit_tickers:
			values = {}
			for i in data[ticker]: # loop over list tuples
				if int(i[0]) in column_periods:
...
def main():
	...
	weekly_pairs = structure_from_csv("creditweeklyupdated.csv", 6)
	weekly_df = create_period_df(weekly_pairs)
	
	monthly_pairs = structure_from_csv("creditmonthlyupdated.csv", 6)
	monthly_df = create_period_df(monthly_pairs)
	
	quarterly_pairs = structure_from_csv("creditquarterlyupdated.csv", 8)
	...
	full_credit_df.to_csv("full_credit.csv")

	# query_1010_server("debit", "quarterly")
	# query_1010_server("debit", "monthly")
	# query_1010_server("debit", "weekly")

	# weekly_pairs = structure_from_csv("debitweeklyupdated.csv", 6)
	# weekly_df = create_period_df(weekly_pairs)
	
	# monthly_pairs = structure_from_csv("debitmonthlyupdated.csv", 6)
	# monthly_df = create_period_df(monthly_pairs)
	
	# quarterly_pairs = structure_from_csv("debitquarterlyupdated.csv", 8)
	# quarterly_df = create_period_df(quarterly_pairs)
	
	# full_debit_df = pd.concat([weekly_df, monthly_df, quarterly_df], axis= 1)
	# print(full_debit_df)
	# full_debit_df.to_csv("full_debit.csv")
